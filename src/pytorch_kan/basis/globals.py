...
class OrthogonalPolynomial:
    ...
    """
    def __init__(self, polynomial:str, order:int, alpha_size: int =None, beta_size: int = None):
        """
        ...
        assert polynomial in self.POLY_WRAPPER.keys(), f"Unsupported polynomial type: {polynomial}"
        ...
        if self.polynomial == "gegenbauer":
            ...
            self.alpha = torch.nn.Parameter(torch.rand(alpha_size, alpha_size), requires_grad=True)

        if self.polynomial == "jacobi":
            assert alpha_size is not None and beta_size is not None, "alpha_size and beta_size required for Jacobi"
            self.activation = torch.nn.SiLU()
    ...
    def _legendre_matrix(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.order > 0:
            ...
            for n in range(2, self.order + 1):
                polys[..., n] = ((2 * n - 1) * x * polys[..., n-1] - (n - 1) * polys[..., n-2]) / n
        return polys
    ...
    def _gegenbauer_matrix(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.order > 0:
            ...
            if self.order > 1:
                ...
                coeff_matrix[:, 0] = -(n_range + 2 * constrained_alpha - 2) / (n_range + 1)
                coeff_matrix[:, 1] = 2 * (n_range + constrained_alpha - 1) / (n_range + 1)
                polys[..., 2:] = torch.einsum('bik,kj->bij', polys[..., 1:-1].unsqueeze(-1) * x.unsqueeze(-1), coeff_matrix[:, 1:]) + \
                                   torch.einsum('bik,k->bi', polys[..., :-2], coeff_matrix[:, 0]).unsqueeze(-1)
        return polys
    ...
    def _hermite_matrix(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.order > 0:
            polys[..., 1] = 2 * x
            for n in range(2, self.order + 1):
                polys[..., n] = 2 * x * polys[..., n-1] - 2 * (n-1) * polys[..., n-2]
        return polys
    ...
    def _jacobi_matrix(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.order > 0:
            ...
            for n in range(2, self.order + 1):
                ...
                B = (2*n + a + b - 1) * (a**2 - b**2 + x*(2*n + a + b - 2)*(2*n + a + b))
