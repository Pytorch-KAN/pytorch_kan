...
class ChebyshevSecondKANLayer(nn.Module):
    def __init__(self, input_dim, output_dim, order=3, grid_epsilon=1e-6):
        super(ChebyshevSecondKANLayer, self).__init__()
        ...
        self.chebyshev_calc = OrthogonalPolynomial(polynomial="chebyshev_second", order=order)
    ...
    def forward(self, x):
        ...
        # Scale input to [-1, 1] range (Chebyshev domain)
        x = torch.clamp(x, -1.0 + self.grid_epsilon, 1.0 - self.grid_epsilon)
        
        # Compute Chebyshev polynomial basis values
        basis_values = self.chebyshev_calc.calculate_basis(x)
...
class ChebyshevSecondKANMNISTModel(nn.Module):
    def __init__(self, hidden_dim=256, order=5):
        super(ChebyshevSecondKANMNISTModel, self).__init__()
        self.flatten = nn.Flatten()
        
        self.kan_layer1 = ChebyshevSecondKANLayer(
            input_dim=784, 
            output_dim=hidden_dim, 
            order=order
        )
        
        self.kan_layer2 = ChebyshevSecondKANLayer(
...
def plot_results(train_losses, test_accuracies, save_path='output/mnist_chebyshev_second_results.png'):
...
def main():
    ...
    model = ChebyshevSecondKANMNISTModel(hidden_dim=256, order=order).to(device)
    
    print(f"Chebyshev Second Kind KAN MNIST model architecture (order = {order}):")
    print(model)
    ...
    torch.save({
        ...
        'order': order
    }, 'output/mnist_chebyshev_second_model.pth')
    print("Model and metrics saved to 'output/mnist_chebyshev_second_model.pth'")
