...
class HermiteKANLayer(nn.Module):
    def __init__(self, input_dim, output_dim, order=3, grid_epsilon=1e-6):
        ...
        # Create Hermite polynomial calculator
        self.hermite_calc = OrthogonalPolynomial(polynomial="hermite", order=order)
    ...
    def forward(self, x):
        ...
        # Scale input to [-4, 4] range (typical for Hermite)
        x = torch.clamp(x, -4.0 + self.grid_epsilon, 4.0 - self.grid_epsilon)
        
        # Compute Hermite polynomial basis values
        basis_values = self.hermite_calc.calculate_basis(x)
...
class HermiteKANMNISTModel(nn.Module):
    def __init__(self, hidden_dim=256, order=5):
        super(HermiteKANMNISTModel, self).__init__()
        self.flatten = nn.Flatten()
        
        self.kan_layer1 = HermiteKANLayer(
            input_dim=784, 
            output_dim=hidden_dim, 
            order=order
        )
        
        self.kan_layer2 = HermiteKANLayer(
...
def plot_results(train_losses, test_accuracies, save_path='output/mnist_hermite_results.png'):
...
def main():
    ...
    model = HermiteKANMNISTModel(hidden_dim=256, order=order).to(device)
    
    print(f"Hermite KAN MNIST model architecture (order = {order}):")
    print(model)
    ...
    torch.save({
        ...
        'order': order
    }, 'output/mnist_hermite_model.pth')
    print("Model and metrics saved to 'output/mnist_hermite_model.pth'")
