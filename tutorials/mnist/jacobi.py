...
class JacobiKANLayer(nn.Module):
    def __init__(self, input_dim, output_dim, order=3, grid_epsilon=1e-6):
        ...
        # Create Jacobi polynomial calculator with learnable alpha and beta
        self.jacobi_calc = OrthogonalPolynomial(
            polynomial="jacobi", 
            order=order,
    ...
    def forward(self, x):
        ...
        # Scale input to [-1, 1] range (Jacobi domain)
        x = torch.clamp(x, -1.0 + self.grid_epsilon, 1.0 - self.grid_epsilon)
        
        # Compute Jacobi polynomial basis values
        basis_values = self.jacobi_calc.calculate_basis(x)
...
class JacobiKANMNISTModel(nn.Module):
    def __init__(self, hidden_dim=256, order=5):
        super(JacobiKANMNISTModel, self).__init__()
        self.flatten = nn.Flatten()
        
        self.kan_layer1 = JacobiKANLayer(
            input_dim=784, 
            output_dim=hidden_dim, 
            order=order
        )
        
        self.kan_layer2 = JacobiKANLayer(
...
def plot_results(train_losses, test_accuracies, save_path='output/mnist_jacobi_results.png'):
...
def main():
    ...
    model = JacobiKANMNISTModel(hidden_dim=256, order=order).to(device)
    
    print(f"Jacobi KAN MNIST model architecture (order = {order}):")
    print(model)
    ...
    torch.save({
        ...
        'order': order
    }, 'output/mnist_jacobi_model.pth')
    print("Model and metrics saved to 'output/mnist_jacobi_model.pth'")
