...
class GegenbauerKANLayer(nn.Module):
    def __init__(self, input_dim, output_dim, order=3, grid_epsilon=1e-6):
        ...
        # Create Gegenbauer polynomial calculator with learnable alpha
        self.gegenbauer_calc = OrthogonalPolynomial(
            polynomial="gegenbauer", 
            order=order,
            alpha_size=input_dim
        )
    ...
    def forward(self, x):
        ...
        # Scale input to [-1, 1] range (Gegenbauer domain)
        x = torch.clamp(x, -1.0 + self.grid_epsilon, 1.0 - self.grid_epsilon)
        
        # Compute Gegenbauer polynomial basis values
        basis_values = self.gegenbauer_calc.calculate_basis(x)
...
class GegenbauerKANMNISTModel(nn.Module):
    def __init__(self, hidden_dim=256, order=5):
        super(GegenbauerKANMNISTModel, self).__init__()
        self.flatten = nn.Flatten()
        
        self.kan_layer1 = GegenbauerKANLayer(
            input_dim=784, 
            output_dim=hidden_dim, 
            order=order
        )
        
        self.kan_layer2 = GegenbauerKANLayer(
...
def plot_results(train_losses, test_accuracies, save_path='output/mnist_gegenbauer_results.png'):
...
def main():
    ...
    model = GegenbauerKANMNISTModel(hidden_dim=256, order=order).to(device)
    
    print(f"Gegenbauer KAN MNIST model architecture (order = {order}):")
    print(model)
    ...
    torch.save({
        ...
        'order': order
    }, 'output/mnist_gegenbauer_model.pth')
    print("Model and metrics saved to 'output/mnist_gegenbauer_model.pth'")
