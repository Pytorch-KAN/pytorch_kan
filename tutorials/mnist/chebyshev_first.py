...
class ChebyshevFirstKANLayer(nn.Module):
    def __init__(self, input_dim, output_dim, order=3, grid_epsilon=1e-6):
        super(ChebyshevFirstKANLayer, self).__init__()
        ...
        self.chebyshev_calc = OrthogonalPolynomial(polynomial="chebyshev_first", order=order)
...
class ChebyshevFirstKANMNISTModel(nn.Module):
    def __init__(self, hidden_dim=256, order=5):
        super(ChebyshevFirstKANMNISTModel, self).__init__()
        self.flatten = nn.Flatten()
        
        self.kan_layer1 = ChebyshevFirstKANLayer(
            input_dim=784, 
            output_dim=hidden_dim, 
            order=order
        )
        
        self.kan_layer2 = ChebyshevFirstKANLayer(
...
def plot_results(train_losses, test_accuracies, save_path='output/mnist_chebyshev_first_results.png'):
...
def main():
    ...
    model = ChebyshevFirstKANMNISTModel(hidden_dim=256, order=order).to(device)
    
    print(f"Chebyshev First Kind KAN MNIST model architecture (order = {order}):")
    print(model)
    ...
    torch.save({
        ...
        'order': order
    }, 'output/mnist_chebyshev_first_model.pth')
    print("Model and metrics saved to 'output/mnist_chebyshev_first_model.pth'")
